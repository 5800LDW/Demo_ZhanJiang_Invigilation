...
class ExaminerLoginActivity : BaseActivity() {

    private var mHandler: Handler? = null
    private var dynamicPermission: DynamicPermission? = null

    private var activity: ExaminerLoginActivity? = null


    override fun getLayoutId(): Int {
        return R.layout.activity_examiner_login
    }

    override fun setTitleBar(titleBar: TitleBar) {
        titleBar.setTitle(R.string.app_examiner_login)
    }
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        //权限判断
        dynamicPermission =
            DynamicPermission(this@ExaminerLoginActivity, DynamicPermission.PassPermission { })
        ...
        mHandler = MyHandler(this@ExaminerLoginActivity)
        mHandler!!.sendEmptyMessage(ConstantList.MESSAGE_VALID_NFCSTART)
        nfcCardReaderAPI = NfcCardReader(mHandler, this@ExaminerLoginActivity)
        bluetoothReaderAPI = BluetoothReader(this@ExaminerLoginActivity, mHandler)
    }
    ...
    private class MyHandler(activity: ExaminerLoginActivity) : Handler() {
        private val activityWeakReference: WeakReference<ExaminerLoginActivity>
        //蓝牙nfc初始化之后不再初始化
...
//                    Toast.makeText(activity, "读卡成功", Toast.LENGTH_SHORT).show()
                    ...
                    if (card != null) {
                        ...
                        if (proctorDetailsBean == null) {
                            activity?.showErrorMessageDialog2("登录失败！您的身份证信息没有绑定！请联系管理员。")
                        } else {
